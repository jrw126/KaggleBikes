...
# exploratory graphs
xyplot(expm1(casual) ~ 1:nrow(train) | day, data = train, group = humidity_b)
...
# model formula
trainVars <- "~ year + season + month + day + 
...
rf.fit.cas <- randomForest(as.formula(paste("casual", trainVars)), train,
                           ntree = 1000, mtry = 6)
rf.fit.cnt <- randomForest(as.formula(paste("count", trainVars)), train,
                           ntree = 1000, mtry = 6)

rf.pred.reg <- expm1(predict(rf.fit.reg, test))
rf.pred.cas <- expm1(predict(rf.fit.cas, test))
rf.pred.cnt <- expm1(predict(rf.fit.cnt, test))

# get 95th percentiles by hour
# top.reg <- data.frame(
#            aggregate(expm1(registered) ~ hour, train, function(x) quantile(x, probs = .95)))
# top.cas <- data.frame(
#            aggregate(expm1(casual) ~ hour, train, function(x) quantile(x, probs = .95)))
# tops <- merge(merge(test, top.reg), top.cas)[, c(2, 43:44)]
# names(tops) <- c("hour", "top.reg", "top.cas")

# both <- data.frame(ifelse(rf.pred.reg > tops$top.reg, tops$top.reg, rf.pred.reg), 
#                    ifelse(rf.pred.cas > tops$top.cas, tops$top.cas, rf.pred.cas))
both <- data.frame(rf.pred.reg, rf.pred.cas)

preds <- rowMeans(data.frame(rowSums(both, na.rm = T), rf.pred.cnt), na.rm = T)

# submission output file for rf
submit_rf <- data.frame(datetime = test$datetime, count = as.integer(preds))
# write.csv(submit_rf, "bag_20150302.csv", row.names = F)
